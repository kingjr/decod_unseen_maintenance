...
def pairwise(X, Y, func, n_jobs=-1):
    """WIP This aims at applying pairwise operation on two matrices using
     multicore e.g. f(X[:, jj, kk], Y[:, jj, kk], np.sum)

    Parameters
    ----------
        X : np.array, shape(n, ...)  # FIXME, shouldn't need this first dim
        Y : np.array, shape(n, ...)
        func : function
    Returns
    -------
