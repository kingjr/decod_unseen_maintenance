...
for f = 1:length(FOIs)
    ...
    for s = 1:length(SubjectsList)
        ...
        file_ft         = [data_path 'preprocessed/' subject '_preprocessed_Tfoi_mtm_' num2str(round(foi)) 'Hz.mat'];
        load(file_ft, 'data');
...
for s = 1:length(SubjectsList)
    ...
    cfg         = [];
    cfg.clf_type= 'SVR';
    cfg.contrast = 'targetAngle';
    [cos_angle sin_angle] = decode_defineContrast(cfg,trials);
    ...
    results_x               = jr_classify(X(:,coi,toi),cos_angle,cfg);
    % predict y axis of orientation
    results_y               = jr_classify(X(:,coi,toi),sin_angle,cfg);
