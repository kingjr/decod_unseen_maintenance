...
from config import (subjects, pyoutput_path)
...
# PLOT
# ------ Plot GAT
gat.scores_ = np.mean(np.array(angle_errors), axis=0)
...
for vis in arange(4):
    ...
    plt.figure(2)
    times = gat.train_times['times_']
    scores_diag = np.transpose([angle_errors_vis[0:19, t, t,vis] for t in range(len(times))])
...
for vis in arange(4):
    # define X
    X = angle_errors_vis[:,:,:,vis] - np.pi /6.
